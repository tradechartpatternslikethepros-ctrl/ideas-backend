    ...
    try {
      switch (op) {
        case 'like_toggle': {
          ...
          return res.json({ ok: true, id, likeCount: count, liked });
        }
        case 'comment_list': {
          const id = String(body.id || '');
...
app.patch('/ideas/:id', (req, res) => {
  const it = getIdea(req.params.id);
  if (!it) return res.status(404).json({ error: 'Not found' });
  const { title, symbol, levelText, take, imageUrl, imageData, type } = req.body || {};
  const next = {
    ...it,
...
app.get('/ideas/:id/comments', (req, res) => {
  res.json(listComments(String(req.params.id)));
});
...
app.put('/ideas/:id/likes', (req, res) => {
  ...
  res.json({ ok: true, likeCount: count, liked: target });
});
...
function likeSetFromBody(req, res, value){
  ...
  res.json({ ok: true, likeCount: count, liked: !!like });
}
